...
const func: DeployFunction = async function ({ getNamedAccounts, deployments, ...hre }: HardhatRuntimeEnvironment) {
  ...
  const { OracleQuoteUnit, BaseCurrency } = poolConfig as ICommonConfiguration;

  const { address: addressesProviderAddress } = await deployments.get(POOL_ADDRESSES_PROVIDER_ID);

  const baseCurrencyToken = BaseCurrency?.[network] ?? ZERO_ADDRESS;
  ...
  await deploy(ORACLE_ID, {
    from: deployer,
    args: [addressesProviderAddress, assets, sources, fallbackOracleAddress, baseCurrencyToken, parseUnits('1', OracleQuoteUnit)],
