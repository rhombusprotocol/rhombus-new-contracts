...
import { INCENTIVES_PULL_REWARDS_STRATEGY_ID, INCENTIVES_V2_IMPL_ID, INCENTIVES_PROXY_ID } from '../../helpers/deploy-ids';
import { HardhatRuntimeEnvironment } from 'hardhat/types';
import { DeployFunction } from 'hardhat-deploy/types';
import { COMMON_DEPLOY_PARAMS } from '../../helpers/env';
import { ConfigNames, eNetwork, getContract, loadPoolConfig, waitForTx } from '../../helpers';
...
const func: DeployFunction = async function ({ getNamedAccounts, deployments, ...hre }: HardhatRuntimeEnvironment) {
  ...
  const addressesProviderInstance = (await getContract('PoolAddressesProvider', addressesProvider)).connect(
  ...
  const emissionManager = (await hre.ethers.getContractAt(emissionManagerArtifact.abi, emissionManagerArtifact.address)) as EmissionManager;
  ...
  const incentivesImpl = (await hre.ethers.getContractAt(incentivesImplArtifact.abi, incentivesImplArtifact.address)) as RewardsController;
  ...
  const isRewardsProxyPending = (await addressesProviderInstance.getAddress(incentivesControllerId)) === ZERO_ADDRESS;
  ...
  const { address: rewardsProxyAddress } = await deployments.get(INCENTIVES_PROXY_ID);

  // Init RewardsController address
  await waitForTx(await emissionManager.setRewardsController(rewardsProxyAddress));
  ...
  if (!isLive) {
    ...
    if (stakedAaveAddress) {
      await deploy(INCENTIVES_STAKED_TOKEN_STRATEGY_ID, {
        ...
        args: [rewardsProxyAddress, incentivesEmissionManager, stakedAaveAddress],
        ...COMMON_DEPLOY_PARAMS,
      });
    } else {
  ...
  await waitForTx(await emissionManager.transferOwnership(incentivesEmissionManager));
