...
const func: DeployFunction = async function ({ getNamedAccounts, deployments, ...hre }: HardhatRuntimeEnvironment) {
  ...
  const addressesProviderInstance = (await getContract('PoolAddressesProvider', addressesProvider)) as PoolAddressesProvider;
  ...
  const aToken = (await hre.ethers.getContractAt(aTokenArtifact.abi, aTokenArtifact.address)) as AToken;
  await waitForTx(
  ...
  const delegationAwareATokenArtifact = await deploy(DELEGATION_AWARE_ATOKEN_IMPL_ID, {
  ...
  const stableDebtToken = (await hre.ethers.getContractAt(stableDebtTokenArtifact.abi, stableDebtTokenArtifact.address)) as StableDebtToken;
  await waitForTx(
