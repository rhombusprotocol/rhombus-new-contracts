...
const func: DeployFunction = async function ({ getNamedAccounts, deployments, ...hre }: HardhatRuntimeEnvironment) {
  ...
  const poolImplDeployment = isL2PoolSupported(poolConfig) ? await deployments.get(L2_POOL_IMPL_ID) : await deployments.get(POOL_IMPL_ID);
  ...
  if (isPoolProxyPending) {
    const setPoolImplTx = await waitForTx(await addressesProviderInstance.setPoolImpl(poolImplDeployment.address));
  ...
  const isPoolConfiguratorProxyPending = (await addressesProviderInstance.getPoolConfigurator()) === ZERO_ADDRESS;
  ...
  if (isPoolConfiguratorProxyPending) {
    const setPoolConfiguratorTx = await waitForTx(
      await addressesProviderInstance.setPoolConfiguratorImpl(poolConfiguratorImplDeployment.address)
    );
  ...
  const poolConfiguratorInstance = (await getPoolConfiguratorProxy()).connect(await hre.ethers.getSigner(deployer));

  // Set total Flash Loan Premium
  await waitForTx(await poolConfiguratorInstance.updateFlashloanPremiumTotal(poolConfig.FlashLoanPremiums.total));
  // Set protocol Flash Loan Premium
  await waitForTx(await poolConfiguratorInstance.updateFlashloanPremiumToProtocol(poolConfig.FlashLoanPremiums.protocol));
