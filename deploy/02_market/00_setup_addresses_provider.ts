...
import { checkRequiredEnvironment, ConfigNames, getReserveAddresses, loadPoolConfig } from '../../helpers/market-config-helpers';
...
const func: DeployFunction = async function (hre: HardhatRuntimeEnvironment) {
  ...
  const addressesProviderInstance = (
    (await hre.ethers.getContractAt(addressesProviderArtifact.abi, addressesProviderArtifact.address)) as PoolAddressesProvider
  ).connect(signer);
  ...
  const protocolDataProvider = await deploy(POOL_DATA_PROVIDER, {
    from: deployer,
    contract: 'contracts/misc/AaveProtocolDataProvider.sol:AaveProtocolDataProvider',
  ...
  if (!isEqualAddress(protocolDataProvider.address, currentProtocolDataProvider)) {
    await waitForTx(await addressesProviderInstance.setPoolDataProvider(protocolDataProvider.address));
  }
