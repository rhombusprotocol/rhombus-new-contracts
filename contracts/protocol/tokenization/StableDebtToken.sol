    ...
    function burn(address from, uint256 amount) external virtual override onlyPool returns (uint256, uint256) {
        ...
        if (previousSupply <= amount) {
            ...
            if (secondTerm >= firstTerm) {
                ...
                nextAvgStableRate =
                    _avgStableRate = ((firstTerm - secondTerm).rayDiv(nextSupply.wadToRay())).toUint128();
            }
        }
        ...
        if (balanceIncrease > amount) {
            ...
            emit Mint(
                from, from, amountToMint, currentBalance, balanceIncrease, userStableRate, nextAvgStableRate, nextSupply
            );
        } else {
