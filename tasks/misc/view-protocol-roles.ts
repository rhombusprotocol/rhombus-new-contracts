...
import { EMERGENCY_ADMIN, POOL_ADMIN, ZERO_ADDRESS } from './../../helpers/constants';
import { getEmissionManager, getIncentivesV2, getOwnableContract } from './../../helpers/contract-getters';
import { EMISSION_MANAGER_ID } from './../../helpers/deploy-ids';
import { FORK } from '../../helpers/hardhat-config-helpers';
import { TREASURY_PROXY_ID, TREASURY_CONTROLLER_ID } from '../../helpers/deploy-ids';
...
task(`view-protocol-roles`, `View current admin of each role and contract`).setAction(async (_, hre) => {
  // Deployer admins
  ...
  const aclManager = (await getACLManager(await poolAddressesProvider.getACLManager())).connect(aclSigner);
  ...
  const paraswapSwapAdapter = await getOwnableContract(await (await hre.deployments.get('ParaSwapLiquiditySwapAdapter')).address);
  const paraswapRepayAdapter = await getOwnableContract(await (await hre.deployments.get('ParaSwapRepayAdapter')).address);
  const paraswapWithdrawSwapAdapter = await getOwnableContract(await (await hre.deployments.get('ParaSwapWithdrawSwapAdapter')).address);
  ...
  const result = [
    ...
    },
    {
      ...
      address: (await aclManager.hasRole(hre.ethers.constants.HashZero, deployer)) ? 'NOT REVOKED' : 'REVOKED',
    ...
    },
    {
      ...
      address: (await aclManager.isPoolAdmin(desiredAdmin)) ? desiredAdmin : ZERO_ADDRESS,
    ...
    {
      role: 'EmergencyAdmin',
      address: (await aclManager.isEmergencyAdmin(desiredEmergencyAdmin)) ? desiredEmergencyAdmin : emergencyAdmin,
    ...
    },
    {
      ...
      address: emissionManager ? await emissionManager.owner() : 'Missing contract address',
      assert: emissionManager ? (await emissionManager.owner()) === desiredAdmin : false,
    },
    {
      role: 'Treasury Proxy Admin',
      address: await getProxyAdminBySlot(treasuryProxy.address),
      assert: (await getProxyAdminBySlot(treasuryProxy.address)) === desiredAdmin,
    },
    {
    ...
    },
    {
      ...
      assert: (await poolAddressesProvider.getAddress(incentivesControllerId)) === rewardsController.address,
    },
    {
