import { getOracleByAsset, getReserveAddress } from '../../helpers/market-config-helpers';
...
task(`setup-e-modes`, `Setups e-modes from config`).setAction(async (_, hre) => {
  ...
  for (let key in config.EModes) {
    ...
    await waitForTx(
      await poolConfigurator.setEModeCategory(eMode.id, eMode.ltv, eMode.liquidationThreshold, eMode.liquidationBonus, oracle, eMode.label)
    );
    ...
    for (let assetIndex in eMode.assets) {
      ...
      await waitForTx(await poolConfigurator.setAssetEModeCategory(assetAddress, eMode.id));
