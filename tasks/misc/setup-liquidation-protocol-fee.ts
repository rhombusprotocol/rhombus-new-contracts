...
task(`setup-liquidation-protocol-fee`, `Setups reserve liquidation protocol fee from configuration`).setAction(async (_, hre) => {
  ...
  for (let asset in config.ReservesConfig) {
    const liquidationProtocolFee = BigNumber.from(config.ReservesConfig[asset].liquidationProtocolFee);
    const assetAddress = await getReserveAddress(config, asset);

    if (liquidationProtocolFee && liquidationProtocolFee.gt('0')) {
      await waitForTx(await poolConfigurator.setLiquidationProtocolFee(assetAddress, liquidationProtocolFee));
