...
import { getAToken, getPoolAddressesProvider } from './../../helpers/contract-getters';
import { ATOKEN_IMPL_ID, INCENTIVES_PROXY_ID, POOL_ADDRESSES_PROVIDER_ID, TREASURY_PROXY_ID } from './../../helpers/deploy-ids';
...
task(`upgrade-atokens`)
  ...
  .setAction(async ({ revision }, { deployments, getNamedAccounts, ...hre }) => {
    ...
    const { ATokenNamePrefix, SymbolPrefix } = await loadPoolConfig(MARKET_NAME);
    ...
    const protocolDataProvider = await getAaveProtocolDataProvider(await poolAddressesProvider.getPoolDataProvider());

    const poolConfigurator = await getPoolConfiguratorProxy(await poolAddressesProvider.getPoolConfigurator());
    ...
    const deployedRevision = await (await (await getAToken(newAtokenArtifact.address)).ATOKEN_REVISION()).toString();
