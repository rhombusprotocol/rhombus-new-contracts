...
task(`review-stable-borrow`, ``)
  ...
  .setAction(async ({ fix, vvv, checkOnly }: { fix: boolean; vvv: boolean; checkOnly: string }, hre) => {
    ...
    console.log(poolConfig.ReservesConfig, reserveAssets);
    if (!reserveAssets) {
    ...
    for (let index = 0; index < reservesToCheck.length; index++) {
      ...
      const expectedStableRateEnabled = poolConfig.ReservesConfig[normalizedSymbol.toUpperCase()].stableBorrowRateEnabled;
      ...
      if (delta) {
        ...
        await waitForTx(await poolConfigurator.setReserveStableRateBorrowing(tokenAddress, expectedStableRateEnabled));
        const newOnChainStableRateEnabled = (await dataProvider.getReserveConfigurationData(tokenAddress)).stableBorrowRateEnabled;
        vvv && console.log('[FIX] Set ', normalizedSymbol, 'Stable Rate Borrowing to', newOnChainStableRateEnabled);
      } else {
        vvv && console.log(chalk.green(`  - Reserve ${normalizedSymbol} Borrow Stable Rate follows the expected configuration`));
        continue;
      }
    }
  });
