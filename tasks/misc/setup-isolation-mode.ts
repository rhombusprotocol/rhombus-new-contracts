...
task(`setup-isolation-mode`, `Setup isolation mode from configuration`).setAction(async (_, hre) => {
  ...
  for (let asset in config.ReservesConfig) {
    ...
    if (assetAddress && borrowableIsolation) {
      await waitForTx(await poolConfigurator.setBorrowableInIsolation(assetAddress, borrowableIsolation));
