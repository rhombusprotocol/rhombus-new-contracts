...
task(`setup-debt-ceiling`, `Setups reserve debt ceiling from configuration`).setAction(async (_, hre) => {
  ...
  for (let asset in config.ReservesConfig) {
    const debtCeiling = BigNumber.from(config.ReservesConfig[asset].debtCeiling);
    const assetAddress = await getReserveAddress(config, asset);

    if (debtCeiling.gt('0')) {
      await waitForTx(await poolConfigurator.setDebtCeiling(assetAddress, debtCeiling));
      console.log(
        '- Updated debt ceiling of',
        asset,
        'at',
        (Number(config.ReservesConfig[asset].debtCeiling) / 100).toLocaleString(undefined, { minimumFractionDigits: 2 })
      );
