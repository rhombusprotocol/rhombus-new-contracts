...
import { POOL_CONFIGURATOR_PROXY_ID, POOL_ADDRESSES_PROVIDER_ID } from '../../helpers/deploy-ids';
import { getAddressFromJson, waitForTx } from '../../helpers/utilities/tx';
import { getOracleByAsset, loadPoolConfig } from '../../helpers/market-config-helpers';
import { getPool, getPoolAddressesProvider, getPoolConfiguratorProxy } from '../../helpers/contract-getters';
...
task(`review-e-mode`, ``)
  ...
  .setAction(async ({ id, fix }: { id: string; fix: boolean }, hre) => {
    ...
    const poolConfigurator = (await getPoolConfiguratorProxy(await getAddressFromJson(network, POOL_CONFIGURATOR_PROXY_ID))).connect(
    ...
    const expectedCategory = {
      ...
      priceSource: configEmodeCategory.oracleId ? await getOracleByAsset(poolConfig, configEmodeCategory.oracleId) : ZERO_ADDRESS,
    ...
    if (delta) {
      console.log(`- Found ${chalk.red('differences')} at on chain Emode versus expected "${id}" EMODE from local configuration`);
      console.log(chalk.red('Current strategy', '=>', chalk.green('Desired strategy from config')));
      ...
      console.log(chalk.green(`  - Emode ID ${id} matches local Emode configuration`));
    }
  });
