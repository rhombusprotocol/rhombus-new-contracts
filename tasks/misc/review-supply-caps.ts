...
import { POOL_CONFIGURATOR_PROXY_ID, POOL_DATA_PROVIDER } from '../../helpers/deploy-ids';
...
task(`review-supply-caps`, ``)
  ...
  .setAction(async ({ fix, checkOnly }: { fix: boolean; checkOnly: string }, hre) => {
    const network = FORK ? FORK : (hre.network.name as eNetwork);
    const { poolAdmin } = await hre.getNamedAccounts();
    const checkOnlyReserves: string[] = checkOnly ? checkOnly.split(',') : [];
    const dataProvider = await getAaveProtocolDataProvider(await getAddressFromJson(network, POOL_DATA_PROVIDER));
    ...
    for (let index = 0; index < reservesToCheck.length; index++) {
      ...
      if (delta) {
        ...
        await waitForTx(await poolConfigurator.setSupplyCap(tokenAddress, expectedSupplyCap));
        ...
        console.log(chalk.green(`  - Reserve ${normalizedSymbol} supply cap follows the expected configuration`));
        continue;
      }
    }
  });
