...
import { EMISSION_MANAGER_ID, POOL_ADDRESSES_PROVIDER_ID } from './../../helpers/deploy-ids';
import {
  getACLManager,
  getEmissionManager,
  getOwnableContract,
...
task(`transfer-protocol-ownership`, `Transfer the ownership of protocol from deployer`).setAction(async (_, hre) => {
  // Deployer admins
  const { poolAdmin, aclAdmin, deployer, emergencyAdmin, incentivesEmissionManager, treasuryProxyAdmin, addressesProviderRegistryOwner } =
  ...
  const isDeployerAdminParaswapWithdrawSwapAdapter = (await paraswapWithdrawSwapAdapter.owner()) == deployer;
  ...
  /** Start of Pool Addresses Provider  Registry transfer ownership */
  const isDeployerPoolAddressesProviderRegistryOwner = (await poolAddressesProviderRegistry.owner()) === addressesProviderRegistryOwner;
  ...
  if (isDeployerDefaultAdmin) {
    ...
    await waitForTx(await aclManager.grantRole(hre.ethers.constants.HashZero, desiredAdmin));
  ...
  const result = [
    {
      role: 'PoolAdmin',
      address: (await aclManager.isPoolAdmin(desiredAdmin)) ? desiredAdmin : poolAdmin,
