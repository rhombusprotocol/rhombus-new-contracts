...
import { getACLManager, getPoolAddressesProvider } from '../../helpers/contract-getters';
...
task(`renounce-pool-admin`, `Renounce PoolAdmin role as deployer if `).setAction(async (_, hre) => {
  ...
  const aclManager = (await getACLManager(await poolAddressesProvider.getACLManager())).connect(deployerSigner);
  ...
  if (isDeployerPoolAdmin && isMultisigPoolAdmin) {
    const tx = await waitForTx(await aclManager.renounceRole(await aclManager.POOL_ADMIN_ROLE(), deployer));
  ...
  const result = [
    {
      ...
      address: (await aclManager.isPoolAdmin(deployer)) ? 'NOT_RENOUNCED' : 'RENOUNCED',
