...
task(`review-rate-strategies`, ``)
  ...
  .setAction(async ({ fix, deploy, checkOnly }: { fix: boolean; checkOnly: string; deploy: boolean }, hre) => {
    ...
    const poolConfigurator = (await getPoolConfiguratorProxy()).connect(await hre.ethers.getSigner(poolAdmin));
    ...
    for (let index = 0; index < reservesToCheck.length; index++) {
      ...
      const expectedStrategy: IInterestRateStrategyParams = poolConfig.ReservesConfig[normalizedSymbol.toUpperCase()].strategy;
      ...
      if (delta) {
        console.log(
          `- Found ${chalk.red('differences')} at reserve ${normalizedSymbol} versus expected "${
        ...
        if (deploy) {
          ...
          if (fix) {
            await waitForTx(await poolConfigurator.setReserveInterestRateStrategyAddress(tokenAddress, fixedInterestStrategy.address));
            console.log('  - Updated Reserve Interest Strategy of', normalizedSymbol, 'at', fixedInterestStrategy.address);
          }
        }
      } else {
        console.log(chalk.green(`  - Reserve ${normalizedSymbol} Interest Rate Strategy matches the expected configuration`));
        continue;
      }
    }
  });
