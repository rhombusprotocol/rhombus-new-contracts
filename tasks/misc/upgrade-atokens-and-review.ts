...
task(`upgrade-atokens-and-review`)
  ...
  .setAction(async ({ revision }, { deployments, getNamedAccounts, ...hre }) => {
    const previousATokenConfigs: { [key: string]: ATokenConfig } = await hre.run('review-atokens', { log: true });
    ...
    const afterATokensConfig: { [key: string]: ATokenConfig } = await hre.run('review-atokens', { log: true });
